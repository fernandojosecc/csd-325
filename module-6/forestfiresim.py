    ...
    while True:  # Main program loop.
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                # Add logic for water: it should never change, and it should prevent fire from spreading through it.
                if forest[(x, y)] == WATER:
...
def createNewForest():
    ...
    lake_x_range = range(WIDTH // 2 - 5, WIDTH // 2 + 5)
    lake_y_range = range(HEIGHT // 2 - 2, HEIGHT // 2 + 3)
    for x in range(WIDTH):
        for y in range(HEIGHT):
            # Check if the current position is within the lake boundaries
            if x in lake_x_range and y in lake_y_range:
                forest[(x, y)] = WATER
